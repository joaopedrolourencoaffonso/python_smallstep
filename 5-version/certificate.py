...
--> ''');
        ...
        url = "https://192.168.0.155:8000/registration/" + number;#my step ca is installed on a Oracle VM, so I reserved an IP for it.
        ...
        if str(response.text) == "1111":
            ...
            '''
            I am still struggling to make python requests to accept the self signed certificate, so for now I am going to use
            "verify=false", but at least the SSL protocol is enabled.
            '''

            url = "https://192.168.0.155:8000/send_token/" + number + ":" + token;
            response = requests.get(url, verify=False);
            ...
            comand = 'openssl req -new -key {number}.pem -out {number}.csr -subj "/C=ZZ/ST=something/L=something/O=something/OU=something/CN={number}"';
